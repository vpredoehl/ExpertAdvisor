...
[[ "$h1_sum" == "86a35844edd3cc233e8f72ff985c339474dc09d3cd79d354fcb3adeb902aa66f" ]] || fail 55000 H2A004 local-055 "migration-055-bytes-changed"
...
result="$(psql -X -qAt -v ON_ERROR_STOP=1 -h "$host" -p "$port" -U "$user_name" "$database_name" -v h1_sum="$h1_sum" -v h2_sum="$h2_sum" -v h2_bind_sum="$h2_bind_sum" -f "$repo_root/Tests/CampaignOperationsPhaseH2DeploymentAudit.sql" 2>&1)" || {
  ...
  case "$code" in H2A004|H2A005) fail 55000 "$code" "$first" "$first";; H2A001|H2A002|H2A003|H2A006) fail 42501 "$code" "$first" "$first";; *) fail 55000 H2A004 "$first" "$result";; esac
}
if [[ -n "$result" ]]; then
  first="$(printf '%s\n' "$result" | sed -n '1p')"
  code="$(printf '%s' "$first" | cut -d: -f1)"
...
printf 'H2_DEPLOYMENT_AUDIT_OK stage=%s audit_version=%s h1_checksum=%s h2_checksum=%s h2_bind_checksum=%s\n' "$stage" "$audit_version" "$h1_sum" "$h2_sum" "$h2_bind_sum"
