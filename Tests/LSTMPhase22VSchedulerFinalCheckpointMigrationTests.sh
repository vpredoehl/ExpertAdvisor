...
rg -U -q 'schedulerMaterializationRead\.commit\(\);[\s\S]*EA::ModelInputPreparation::Prepare\([\s\S]*CreateLstmForRuntimeLogLevel\([\s\S]*ApplyPersistedModelMaterialization\(' "${main_file}"
