...
test_dir="$(mktemp -d /tmp/ea_lstm_causal_return_sign_persistence.XXXXXX)"
...
"${CXX:-c++}" -std=c++20 -mmacosx-version-min=26.2 -Wall -Wextra -Werror \
    ...
    "${repo_root}/Tests/LSTMCausalReturnSignPersistenceTests.cpp" \
    ...
    -o "${test_dir}/LSTMCausalReturnSignPersistenceTests"
"${test_dir}/LSTMCausalReturnSignPersistenceTests"
