...
test_dir="$(mktemp -d /tmp/ea_lstm_true_session_phase.XXXXXX)"
...
"${CXX:-c++}" -std=c++20 -mmacosx-version-min=26.2 -Wall -Wextra -Werror \
    ...
    "${repo_root}/Tests/LSTMTrueSessionPhaseTests.cpp" \
    ...
    -o "${test_dir}/LSTMTrueSessionPhaseTests"
"${test_dir}/LSTMTrueSessionPhaseTests"
