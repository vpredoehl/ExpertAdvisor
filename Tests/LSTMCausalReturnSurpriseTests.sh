...
test_dir="$(mktemp -d /tmp/ea_lstm_causal_return_surprise.XXXXXX)"
...
"${CXX:-c++}" -std=c++20 -mmacosx-version-min=26.2 -Wall -Wextra -Werror \
    ...
    "${repo_root}/Tests/LSTMCausalReturnSurpriseTests.cpp" \
    ...
    -o "${test_dir}/LSTMCausalReturnSurpriseTests"
"${test_dir}/LSTMCausalReturnSurpriseTests"
