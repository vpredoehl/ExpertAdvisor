...
test_dir="$(mktemp -d /tmp/ea_lstm_causal_close_location.XXXXXX)"
...
"${CXX:-c++}" -std=c++20 -mmacosx-version-min=26.2 -Wall -Wextra -Werror \
    ...
    "${repo_root}/Tests/LSTMCausalCloseLocationTests.cpp" \
    ...
    -o "${test_dir}/LSTMCausalCloseLocationTests"
"${test_dir}/LSTMCausalCloseLocationTests"
