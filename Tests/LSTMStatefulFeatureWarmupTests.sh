...
test_dir="$(mktemp -d /tmp/ea_lstm_stateful_feature_warmup.XXXXXX)"
...
"${CXX:-c++}" -std=c++20 -mmacosx-version-min=26.2 -Wall -Wextra -Werror \
    ...
    -I"${repo_root}/Headers" \
    "${include_flags[@]}" \
    "${repo_root}/Tests/LSTMStatefulFeatureWarmupTests.cpp" \
    ...
    -o "${test_dir}/LSTMStatefulFeatureWarmupTests"
"${test_dir}/LSTMStatefulFeatureWarmupTests"
