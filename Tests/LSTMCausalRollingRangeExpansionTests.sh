...
test_dir="$(mktemp -d /tmp/ea_lstm_causal_rolling_range_expansion.XXXXXX)"
...
"${CXX:-c++}" -std=c++20 -mmacosx-version-min=26.2 -Wall -Wextra -Werror \
    ...
    "${repo_root}/Tests/LSTMCausalRollingRangeExpansionTests.cpp" \
    ...
    -o "${test_dir}/LSTMCausalRollingRangeExpansionTests"
"${test_dir}/LSTMCausalRollingRangeExpansionTests"
