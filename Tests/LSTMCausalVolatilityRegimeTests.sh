...
test_dir="$(mktemp -d /tmp/ea_lstm_causal_volatility_regime.XXXXXX)"
...
"${CXX:-c++}" -std=c++20 -mmacosx-version-min=26.2 -Wall -Wextra -Werror \
    ...
    "${repo_root}/Tests/LSTMCausalVolatilityRegimeTests.cpp" \
    ...
    -o "${test_dir}/LSTMCausalVolatilityRegimeTests"
"${test_dir}/LSTMCausalVolatilityRegimeTests"
