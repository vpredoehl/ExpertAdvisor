...
test_dir="$(mktemp -d /tmp/ea_tensor_phase19c_causal.XXXXXX)"
...
"${CXX:-clang++}" -std=c++20 -mmacosx-version-min=26.2 \
    ...
    "${include_flags[@]}" \
    "${repo_root}/Tests/TensorPhase19CCausalAdapterTests.cpp" \
    "${repo_root}/Sources/StrategyEvaluationAdapters/TensorPhase19CCausalAdapter.cpp" \
    ...
    -L"${repo_root}/DerivedData/ExpertAdvisor/Build/Products/Debug" \
    -lMetaNN -lMetalBuffer -framework Metal -framework Foundation \
    -o "${test_dir}/TensorPhase19CCausalAdapterTests"

"${test_dir}/TensorPhase19CCausalAdapterTests"
printf '%s\n' "TensorPhase19CCausalAdapterTests passed"
