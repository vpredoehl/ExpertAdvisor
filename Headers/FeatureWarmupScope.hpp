//
//  FeatureWarmupScope.hpp
//  ExpertAdvisor
//
//  The Tensor feature builder owns recursive EMA/ATR state in addition to
//  finite rolling state.  Exact values at a requested output boundary therefore
//  require every predecessor observation available from the authoritative
//  candlestick source; no finite bar count is sufficient for the recursive
//  states.
//
...
// This is persisted scientific configuration, not a performance hint.  The
// legacy value preserves the historical cold-start-at-logical-boundary
// interpretation for old experiments and models.  New work must explicitly
// use the full predecessor history required by recursive feature state.
