    ...
    DownstreamControlOwnerId controlOwnerId, long long experimentId)
{
    ...
    const auto status = current[0].as<std::string>();
    const auto phase = current[1].as<std::string>();
    ...
    std::ostringstream canonical;
    canonical.imbue(std::locale::classic());
    canonical << "experiment_lifecycle_cancellation_event_v1"
        << ";cancellation_request_id="
        << cancellation.cancellationRequestId.value()
    ...
    const auto identity = Identity(canonical.str());
    const auto row = transaction.exec(
        ...
        pqxx::params{cancellation.cancellationRequestId.value(),
            ...
            cancellation.request.actor.value(), identity.canonicalText(),
            identity.hash()}).one_row();
