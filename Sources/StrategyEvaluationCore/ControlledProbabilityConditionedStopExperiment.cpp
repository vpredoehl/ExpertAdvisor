...
std::string OptionalTimestamp(
    const std::optional<std::int64_t>& value)
{
    return value ? std::to_string(*value) : "NULL";
}

std::string OptionalOrdinal(
...
std::size_t ConfidenceBucketIndex(double confidence)
{
    if (!std::isfinite(confidence) || confidence < 0.0 || confidence > 1.0)
        throw std::runtime_error("phase18a_invalid_normalized_confidence");
...
void RequireThreeWayPairing(
    const AuthoritativeMarketPath& path,
    const std::vector<Phase18AStrategyVariantResult>& variants)
{
    if (variants.size() != 3)
        throw std::runtime_error("phase18a_strategy_count_mismatch");
    for (const auto& variant : variants)
    {
        if (variant.evaluation.sourceContentHash != path.Hash() ||
            ...
            throw std::runtime_error("phase18a_strategy_path_mismatch");
    }
    ...
    for (std::size_t index = 0; index < path.Observations().size(); ++index)
    {
        ...
        if (!observation.probabilities)
            throw std::runtime_error(
                "phase18a_observation_probabilities_missing");
        ...
        for (const auto& variant : variants)
        {
            ...
            {
                throw std::runtime_error(
                    "phase18a_observation_pairing_mismatch");
            }
            ...
            {
                throw std::runtime_error(
                    "phase18a_neutral_action_pairing_mismatch");
            }
        }
        ...
        {
            throw std::runtime_error("phase18a_baseline_pairing_mismatch");
        }
    }
}

Phase18AStrategyMetrics BuildMetrics(
    const Phase18AStrategyVariantResult& variant,
    const std::vector<Phase18AObservationEvidence>& evidence)
{
    ...
    {
        throw std::runtime_error("phase18a_strategy_accounting_mismatch");
    }
    return metrics;
}
...
Phase18APairwiseDelta Pairwise(
    const Phase18AStrategyVariantResult& candidate,
    const Phase18AStrategyVariantResult& reference)
{
    ...
    if (left.observationCount != right.observationCount ||
        ...
        throw std::runtime_error("phase18a_pairwise_population_mismatch");
    return {
...
void AppendStrategyLine(std::ostringstream& output,
                        const ControlledProbabilityConditionedStopExperimentResult& result,
                        const Phase18AStrategyVariantResult& variant)
{
    const auto& metrics = variant.metrics;
    output << "PHASE18A_STRATEGY_RESULT"
...
{
    const ProbabilityConditionedStopLossStrategy conditioned{configuration};
    ...
    ControlledProbabilityConditionedStopExperimentResult result;
    result.configuration = configuration;
    result.experimentIdentityCanonical =
        "controlled_probability_conditioned_stop_experiment_v1;"
        "mapping_identity=" + std::string{kDirectionalProbabilityMappingIdentity} +
        ...
        ";conditioned_strategy_hash=" + conditioned.Identity().hash + ";";
    ...
    result.variants = {
        ...
        {kProbabilityConditionedStopLossOutputIdentity,
         EvaluateStrategy(conditioned, input), {}, {}}};
    RequireThreeWayPairing(marketPath, result.variants);
    ...
    for (const auto& variant : result.variants)
    {
             ...
             index < marketPath.Observations().size(); ++index)
        {
            ...
            if (item.actionable)
            {
                const auto stopDecision = conditioned.StopDecision(observation);
                item.directionalProbability =
                    stopDecision.directionalProbability;
                item.normalizedDirectionalConfidence =
                    stopDecision.normalizedDirectionalConfidence;
                         ...
                         kProbabilityConditionedStopLossOutputIdentity)
                {
                    item.baseStopLogarithmicDistance =
                        configuration.baseStopLogarithmicDistance;
                    item.appliedStopMultiplier = stopDecision.stopMultiplier;
                    item.effectiveStopLogarithmicDistance =
                        stopDecision.effectiveStopLogarithmicDistance;
                }
                if (!execution.entryTimestampUnixSeconds ||
                    ...
                    *execution.exitTimestampUnixSeconds <
                    ...
                    throw std::runtime_error(
                        "phase18a_invalid_holding_duration");
    ...
    result.pairwiseDeltas = {
        Pairwise(result.variants[2], result.variants[0]),
        Pairwise(result.variants[2], result.variants[1]),
        Pairwise(result.variants[1], result.variants[0])};

    std::ostringstream lines;
    lines << "PHASE18A_EXPERIMENT"
          << ",experiment_identity_hash=" << result.experimentIdentityHash
    ...
    for (const auto& variant : result.variants)
    {
             ...
             index < variant.confidenceBuckets.size(); ++index)
        {
            ...
            lines << "PHASE18A_CONFIDENCE_BUCKET"
    ...
    for (const auto& delta : result.pairwiseDeltas)
    {
        lines << "PHASE18A_PAIRWISE_DELTA"
              << ",experiment_identity_hash="
              << result.experimentIdentityHash
    ...
    {
        lines << "PHASE18A_OBSERVATION_EVIDENCE"
              << ",experiment_identity_hash="
              << result.experimentIdentityHash
